...
if __name__ == "__main__":
    ...
    opt.num_epochs = 100
    opt.batch_size = 32
    lr = opt.basic_lr_per_img * opt.batch_size

    lr_scheduler = LRScheduler(opt.scheduler, lr, iters_per_epoch, opt.num_epochs, warmup_epochs=opt.warmup_epochs,
                               warmup_lr_start=opt.warmup_lr, no_aug_epochs=opt.no_aug_epochs,
                               min_lr_ratio=opt.min_lr_ratio)

    x = []
    for epoch in range(1, opt.num_epochs + 1):
        for iter_id in range(iters_per_epoch):
            iteration = (epoch - 1) * iters_per_epoch + iter_id
            lr = lr_scheduler.update_lr(iteration)
            print(epoch, iteration, lr)
            x.append([epoch, iteration, lr])

    plt.figure(0)
    plt.plot([i[0] for i in x], [i[2] for i in x])
    plt.figure(1)
    plt.plot([i[1] for i in x], [i[2] for i in x])
    plt.show()
